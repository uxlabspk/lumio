...
import { Search, RefreshCw, UserPlus, CheckCircle, XCircle } from "lucide-react";
...
export default function EnrollmentPage() {
    ...
    async function enrollStudent(studentId: string, classId: string) {
        ...
        try {
            ...
            setMessage(payload.message || "Student enrolled successfully!");
    ...
    const enrolledCount = data?.students.filter((s) => s.class !== null).length ?? 0;
    const unenrolledCount = data?.students.filter((s) => s.class === null).length ?? 0;

    return (
        <div className="space-y-5">
            <div className="flex items-center justify-between">
                <div>
                    <h1 className="text-xl font-semibold text-zinc-900">Student Enrollment</h1>
                    <p className="text-sm text-zinc-500">
                        Enroll students into classes and manage their enrollment status.
                    </p>
                </div>
            ...
            <div className="grid grid-cols-1 gap-4 md:grid-cols-4">
                ...
                <Card>
                    <CardContent className="p-4">
                        ...
                        <div className="mt-1 flex items-center gap-2">
                            <p className="text-2xl font-semibold text-emerald-600">{enrolledCount}</p>
                ...
                <Card>
                    <CardContent className="p-4">
                        <p className="text-xs text-zinc-500">Unenrolled</p>
                        <div className="mt-1 flex items-center gap-2">
                            <p className="text-2xl font-semibold text-orange-600">{unenrolledCount}</p>
                        ...
                        {data && data.classes.length > 0 ? (
                            data.classes.map((classroom) => (
                                <option key={classroom.id} value={classroom.id}>
                                    {classroom.name}
                                    {classroom.section ? ` ${classroom.section}` : ""} ({classroom._count.students}{" "}
                                    students)
                                </option>
                            ))
                        ) : (
                    ...
                    {loading ? (
                        ...
                        <div className="space-y-2">
                            {filteredStudents.map((student) => (
                                <div
                                    key={student.id}
                                    className="grid grid-cols-1 gap-3 rounded-lg border border-zinc-100 p-3 md:grid-cols-5 md:items-center"
                                >
